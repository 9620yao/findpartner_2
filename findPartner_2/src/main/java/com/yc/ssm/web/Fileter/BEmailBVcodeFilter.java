...
@WebFilter("/partner/forget")
public class BEmailBVcodeFilter extends AbstractFilter {
	...
	public void doFilter(ServletRequest requset, ServletResponse response, FilterChain chain)
		...
		if (srcCode != null && outCode != null && srcCode.intern() == outCode.intern()) {
			...
			session.setAttribute("errorEmail", "验证码错误");
			((HttpServletResponse) response).sendRedirect("/findPartner/page/forgetPassword.jsp");
		}
	}
